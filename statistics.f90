...
IMPLICIT NONE
  ...
  Interface
    Subroutine Talagrand (ensemble, truth, res)
       real, dimension(:), intent(IN) :: ensemble
       real, intent(IN) :: truth
       real, intent(OUT) :: res
    End Subroutine Talagrand
  End Interface

real totalvar, storeVal
real psiMeane(nxx,nyy)
real psiVare(nxx,nyy)
integer time, nM, talCount, status, i, j, k, l
character alll*5, filename*20, timeString*6, particleNo*3
real, dimension(:), Allocatable :: talValues !for storing the array of independent talagrand results
real, dimension(:), Allocatable :: perturbVals !for storing the array of observation error perturbed ensemble members
real, dimension(:), Allocatable :: randomVec !for storing the array of normally distributed random error
real, dimension(:,:), Allocatable :: pdfValues !for storing the array of values for calculating pdfs
real, dimension(:,:), Allocatable ::  qTruth
...
use Sizes
    ...
    real, dimension(:), allocatable :: sortedE
