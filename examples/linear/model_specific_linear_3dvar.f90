!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!! Time-stamp: <2015-10-16 14:38:36 pbrowne>
!!!
...
subroutine configure_model
  ...
  integer :: k,tag,mpi_err,particle
  ...
  pf%time_bwn_obs = 1 !set this so the obs are picked up corrently
  pf%time_obs = 1
  ...
  do k =1,pf%count
     ...
     tag = 1
     call mpi_send(data,2,MPI_INTEGER&
...
subroutine Bhalf(nrhs,x,Qx)
  ...
  bhalfdat(1:3,1) = (/ sqrt(2.0_rk), 0.0_rk, 0.0_rk/)
  bhalfdat(1:3,2) = (/ 0.0_rk, sqrt(3.0_rk), 0.0_rk/)
  bhalfdat(1:3,3) = (/ 0.0_rk, 0.0_rk, sqrt(4.0_rk)/)
...
subroutine solve_b(nrhs,x,Qx)
  ...
  binvdat(1:3,1) = (/ 1.0_rk/2.0_rk,0.0_rk,0.0_rk/)
  binvdat(1:3,2) = (/ 0.0_rk,1.0_rk/3.0_rk,0.0_rk/)
  binvdat(1:3,3) = (/ 0.0_rk,0.0_rk,1.0_rk/4.0_rk/)
