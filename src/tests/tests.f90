...
subroutine R_tests()
  ...
  implicit none
  integer, parameter :: rk = kind(1.0d0)
  real(kind=rk), dimension(obs_dim) :: a,s,q,w,e
  real(kind=rk), dimension(obs_dim,10) :: aa,ss,qq,ww,ee
  ...
  do i = 1,10
     ...
     elseif( l .eq. 9) then
        call NormalRandomNumbers2D(0.0d0,1.0d0,obs_dim,i,aa(:,1:i))
        ss = aa
...
subroutine Q_tests()
  ...
  implicit none
  integer, parameter :: rk = kind(1.0d0)
  real(kind=rk), dimension(state_dim) :: a,s,qqq,w,e
  real(kind=rk), dimension(state_dim,10) :: aa,ss,qq,ww,ee
  real(kind=rk) :: dnrm2,rr
  real(kind=rk), parameter :: pass = 1.0d-15
  real(kind=rk), parameter :: warn = 1.0d-13
  integer :: pfcount,i,k,l
  ...
  do i = 1,10
     ...
     elseif( l .eq. 8) then
        call NormalRandomNumbers2D(0.0d0,1.0d0,state_dim,i,aa(:,1:i))
        ss = aa
     ...
     do k = 1,i
        !qq should be ee
        rr = dnrm2(state_dim,qq(:,k)-ee(:,k),1)
!        write(6,'(A,i2,A,i2,A)',advance='no') 'Test 5 ',i,',',k,': H H^T(0) ... '
        if(rr .lt. pass) then
           !passed the test
           if(k .eq. 1 .and. i .eq. 1) then
...
subroutine HQHTR_tests()
  ...
  implicit none
  integer, parameter :: rk = kind(1.0d0)
  real(kind=rk), dimension(obs_dim) :: a,s,qq,hqha,hqha_r
  real(kind=rk), dimension(state_dim) :: qha,ha
  real(kind=rk) :: dnrm2,rr
  real(kind=rk), parameter :: pass = 1.0d-15
  real(kind=rk), parameter :: warn = 1.0d-13
