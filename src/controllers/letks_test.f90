!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!! Time-stamp: <2016-10-18 15:01:55 pbrowne>
!!!
...
program empire
  ...
  real(kind=kind(1.0d0)), allocatable, dimension(:,:) :: analysis,&
       & forecast,inc
  ...
  allocate(analysis(state_dim,pf%count))
  allocate(forecast(state_dim,pf%count))
  allocate(inc(state_dim,pf%count))
  ...
  do j=1,pf%time_obs
     ...
     case('LS')
        call stochastic_model
        !Now we have a forecast. save it
        forecast = pf%psi
        !do letkf
        call letkf_analysis
        analysis = pf%psi
        ...
        pf%psi = forecast
        call letks_filter_stage
        !now check that filter stage agrees with analysis
        print*,'Difference in letkf and letks_filter = ',sum( (pf%psi&
             &-analysis)**2 )
        call letks_increment(forecast,inc)
        pf%psi = forecast + inc
        print*,'Difference in letkf and letks = ',sum( (pf%psi&
             &-analysis)**2 )
