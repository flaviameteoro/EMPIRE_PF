...
IMPLICIT NONE
real(kind=kind(1.0D+0)), PRIVATE      :: zero = 0.0, half = 0.5, one = 1.0, two = 2.0,   &
...
real(kind=kind(1.0D+0)) :: fn_val

!     Local variables
real(kind=kind(1.0D+0))     :: s = 0.449871, t = -0.386595, a = 0.19600, b = 0.25472,           &
...
real(kind=kind(1.0D+0)), INTENT(IN)    :: s
...
! Local variables
real(kind=kind(1.0D+0)), SAVE  :: c, d
real(kind=kind(1.0D+0))        :: u, v, x
...
!     Local variables
real(kind=kind(1.0D+0))       :: r, x, w
real(kind=kind(1.0D+0)), SAVE :: a, p, c, uf, vr, d
...
real(kind=kind(1.0D+0)), INTENT(IN) :: a
real(kind=kind(1.0D+0))             :: fn_val
...
real(kind=kind(1.0D+0)), INTENT(IN)    :: aa, bb
LOGICAL, INTENT(IN) :: first
real(kind=kind(1.0D+0))                :: fn_val

!     Local variables
real(kind=kind(1.0D+0)), PARAMETER  :: aln4 = 1.3862944
real(kind=kind(1.0D+0))             :: a, b, g, r, s, x, y, z
real(kind=kind(1.0D+0)), SAVE       :: d, f, h, t, c
LOGICAL, SAVE    :: swap
...
!     Local variables
real(kind=kind(1.0D+0)), SAVE      :: s, c, a, f, g
real(kind=kind(1.0D+0))            :: r, x, v

real(kind=kind(1.0D+0)), PARAMETER :: three = 3.0, four = 4.0, quart = 0.25,   &
...
INTEGER, INTENT(IN)   :: n
real(kind=kind(1.0D+0)), INTENT(IN)      :: h(:), d(:)   ! d(n*(n+1)/2)
real(kind=kind(1.0D+0)), INTENT(IN OUT)  :: f(:)         ! f(n*(n+1)/2)
real(kind=kind(1.0D+0)), INTENT(OUT)     :: x(:)
LOGICAL, INTENT(IN)   :: first
INTEGER, INTENT(OUT)  :: ier

!     Local variables
INTEGER       :: j, i, m
real(kind=kind(1.0D+0))          :: y, v
INTEGER, SAVE :: n2
...
real(kind=kind(1.0D+0)), INTENT(IN)    :: h, b
LOGICAL, INTENT(IN) :: first
real(kind=kind(1.0D+0))                :: fn_val

!     Local variables
real(kind=kind(1.0D+0))            :: ym, xm, r, w, r1, r2, x
real(kind=kind(1.0D+0)), SAVE      :: a, c, d, e
real(kind=kind(1.0D+0)), PARAMETER :: quart = 0.25
...
!     .. Scalar Arguments ..
real(kind=kind(1.0D+0)), INTENT(IN)    :: mu
LOGICAL, INTENT(IN) :: first
INTEGER             :: ival
!     ..
!     .. Local Scalars ..
real(kind=kind(1.0D+0))          :: b1, b2, c, c0, c1, c2, c3, del, difmuk, e, fk, fx, fy, g,  &
                 omega, px, py, t, u, v, x, xx
real(kind=kind(1.0D+0)), SAVE    :: s, d, p, q, p0
INTEGER       :: j, k, kflag
LOGICAL, SAVE :: full_init
INTEGER, SAVE :: l, m
!     ..
!     .. Local Arrays ..
real(kind=kind(1.0D+0)), SAVE    :: pp(35)
!     ..
!     .. Data statements ..
real(kind=kind(1.0D+0)), PARAMETER :: a0 = -.5, a1 = .3333333, a2 = -.2500068, a3 = .2000118,  &
                   a4 = -.1661269, a5 = .1421878, a6 = -.1384794,   &
                   a7 = .1250060

real(kind=kind(1.0D+0)), PARAMETER :: fact(10) = (/ 1., 1., 2., 6., 24., 120., 720., 5040.,  &
                                 40320., 362880. /)
...
INTEGER         :: ru, rd
INTEGER, SAVE   :: r0
real(kind=kind(1.0D+0))            :: u, pd, pu
real(kind=kind(1.0D+0)), SAVE      :: odds_ratio, p_r
real(kind=kind(1.0D+0)), PARAMETER :: zero = 0.0, one = 1.0
...
!     ..
!     .. Local Scalars ..
real(kind=kind(1.0D+0))            :: alv, amaxp, f, f1, f2, u, v, w, w2, x, x1, x2, ynorm, z, z2
real(kind=kind(1.0D+0)), PARAMETER :: zero = 0.0, half = 0.5, one = 1.0
INTEGER         :: i, ix, ix1, k, mp
INTEGER, SAVE   :: m
real(kind=kind(1.0D+0)), SAVE      :: p, q, xnp, ffm, fm, xnpq, p1, xm, xl, xr, c, al, xll,  &
                   xlr, p2, p3, p4, qn, r, g
...
real(kind=kind(1.0D+0)), INTENT(IN)   :: sk, p
INTEGER            :: ival

!     Local variables
! THE PARAMETER ULN = -LOG(MACHINE'S SMALLEST REAL NUMBER).

real(kind=kind(1.0D+0)), PARAMETER    :: h = 0.7
real(kind=kind(1.0D+0))               :: q, x, st, uln, v, r, s, y, g
INTEGER            :: k, i, n
...
real(kind=kind(1.0D+0)), INTENT(IN)     :: k
LOGICAL, INTENT(IN)  :: first
real(kind=kind(1.0D+0))                 :: fn_val
...
INTEGER          :: j, n
INTEGER, SAVE    :: nk
real(kind=kind(1.0D+0)), PARAMETER  :: pi = 3.14159265
real(kind=kind(1.0D+0)), SAVE       :: p(20), theta(0:20)
real(kind=kind(1.0D+0))             :: sump, r, th, lambda, rlast
REAL (dp)        :: dk
...
real(kind=kind(1.0D+0)), INTENT(IN)      :: a, b
real(kind=kind(1.0D+0)), INTENT(OUT)     :: result
